...
def build_fallback_message(now: datetime | None = None) -> str:
    ...
    for header, feed_list in groups:
        items = _gather(feed_list, config.FALLBACK_ITEMS)
        lines.append(header)
        if not items:
...
def run_fallback(send: bool = True) -> str:
