...
2. **선별 + 요약 (단일 추론 패스, 구독 모델):**
   ...
   - **Smart Brevity:** `one_liner`는 제목을 그대로 반복하지 말고 제목에 없는 핵심 사실(수치·주체·
     결과)을 더한다. 제목과 사실상 동일한 한줄요약은 `--finalize` 렌더 단계에서 자동 제거된다.
     `why_it_matters`는 'Why it matters' 프레임에 맞춰 독자 함의를 **한 문장**으로만 쓴다.
   - **종합(입체 읽기):** 같은 주제인데 수치만 상충하는 항목(예: 영업이익 10조 vs 12조)은 억지로
     하나로 합치지 말고 각각 남긴다. `--finalize`가 `synthesize`로 근접 시각·동일 주제·수치 충돌
     항목을 자동으로 하나의 `종합` 항목으로 묶어 각 출처 수치를 나란히 노출한다.
