...
수집(A·B·C 병렬, 소스 격리) → 정규화(공통 모델)
  ...
  → 종합(동일 주제·수치 충돌 항목 입체 병합)
  → 구조화 요약(1회 LLM 호출, 근거 강제 + Smart Brevity)
...
신뢰도 · 핵심개체 · 원문제목 + 다운스트림 보강 필드(한줄요약/왜중요/함의/종합포인트/태그/근거/플래그).
...
### 요약 — 근거 강제 + Smart Brevity (`news/summarize.py`)
...
Axios의 **Smart Brevity** 원칙을 따라 (1) `한줄요약`은 제목을 반복하지 않고 제목에 없는
핵심 사실을 더하며(제목과 사실상 동일하면 렌더 단계에서 자동 제거), (2) `왜 중요한가
(Why it matters)`는 한 문장 인사이트로 명시 라벨과 함께 분리 표시합니다.

### 종합(입체 읽기) — 동일 주제 다출처 병합 (`news/dedup.py: synthesize`)
과병합 방지가 **수치 충돌**(예: 삼성 영업이익 10조 vs 12조)을 별개 기사로 남기는 것은 안전상
옳지만, 하나의 사안이 두 항목으로 쪼개져 읽기 흐름을 끊습니다. `synthesize`는 선별 이후
**같은 주제·근접 시각·수치만 상충**하는 대표 기사들만 골라 하나의 항목으로 합치고, 각 출처의
수치를 `종합` 블록에 나란히(`이데일리: 10조 · 뉴시스: 12조`) 노출해 입체적으로 읽게 합니다.
`deduplicate()`의 과병합 가드와는 분리되어 있어 안전 규칙은 그대로 유지됩니다.
...
python run.py --finalize    # 렌더 + 발송 + 아카이브 (selection 없으면 휴리스틱 대체)
```

`run.py`는 데이터 문제로 인한 소프트 실패에도 폴백 발송을 시도한 뒤 **항상 exit 0**으로
종료해, 스케줄러가 하드 실패로 처리하지 않게 합니다. `--dry-run`은 텔레그램 발송과
`sent_log` 기록을 모두 건너뛰어 레포 상태를 변경하지 않습니다.

### 신뢰성 보장 (재점검 반영)
- **이중 발송 방지:** 메인 브리핑이 한 청크라도 전송되면 폴백을 트리거하지 않습니다. 텔레그램
  발송은 청크별 지수 백오프 재시도를 거칩니다.
...
news/
  ...
  dedup.py        URL 정규화 + 제목 셰글링 + 과병합 방지 + 종합(입체 병합)
  select.py       1회 LLM 선별 + 다양성 캡 (휴리스틱 폴백)
  summarize.py    1회 LLM 구조화 요약(Smart Brevity) + 번역 (무추론 폴백)
